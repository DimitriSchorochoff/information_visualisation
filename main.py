...
class Ui_MainWindow(object):
    ...
    def init_list_attr(self):
        ...
        attr_lst.append(build_graph.Attribute_categorical("Minimum spanning tree", False, build_graph.attr_find_min_spanning_tree))
    ...
    def color_shortest_path_factory(self, source, graph, method='dijkstra'):
        def color_shortest_path_factory():
            ...
            color = build_graph.get_node_color(graph, source)
            ...
            for i in range(len(path) - 1):
                build_graph.change_node_color(graph, path[idx], color)
                idx += 1
                build_graph.change_edge_color(graph, path[i], path[i + 1], color)
                build_graph.change_edge_width(graph, path[i], path[i+1], new_edge_size)
            build_graph.change_node_color(graph, path[idx], color)
